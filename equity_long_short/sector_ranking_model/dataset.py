...
class RankingDataset(Dataset):
    ...
    def __init__(self, tickers_dir, fundamentals_filename, sectors, series_length, lookback, min_sequence_length, template='*.csv', transform=None,
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            def extract_series(returns, start_index):
                series = returns[start_index: start_index + self.series_length]
                # Add extra feature dimension.
                series = series[..., np.newaxis]
                # Convert to torch tensor.
                series = torch.from_numpy(series)
                return series
            series_1 = extract_series(returns_1, start)
            series_2 = extract_series(returns_2, start)
...
def create_ranking_dataset(tickers_dir, fundamentals_filename, sectors, series_length, lookback, min_sequence_length,
                          start_date=None, end_date=None, fixed_start_date=False,
                          normalised_returns=False, max_n_files=None):
    dataset = RankingDataset(tickers_dir, fundamentals_filename, sectors, series_length, lookback, min_sequence_length,
...
def test_ranking_dataset():
    ...
    dataset = create_ranking_dataset(tickers_dir, fundamentals_filename, sectors, series_length, lookback, min_sequence_length,
